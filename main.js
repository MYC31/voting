...
async function main() {
  ...
  app.post('/createCampaign', async (req, res) => {
    const { reward, candidacyFee, endVoteCount, account, } = req.body;

    if (!reward || !candidacyFee || !endVoteCount || !account) {
        return res.status(400).send({ error: 'Missing parameters' });
    }
  ...
  app.post('/closeCampaign', async (req, res) => {
    const { campaignId, account, } = req.body;
...
app.post('/declareCandidacy', async (req, res) => {
  ...
  console.log("typeof account: ", typeof _account);
  console.log("campaignId: ", campaignId);
  console.log("account: ". _account);
  console.log("/declareCandidacy----------------------\n");
  ...
  web3.eth.getAccounts(function (error, accounts) {
    ...
    App.contracts.Adoption.deployed().then(function (instance) {
      ...
      const reply = adoptionInstance.declareCandidacy(campaignId, {
            from: account,
            value: web3.toWei('0.1', 'ether'), // Assuming the candidacy fee is 0.1 ETH
            gas: 2000000
          });
      return reply;
    }).then(function() {
